...
@section('content')
	<div class="main cf">
		...
		@endif
		<form action="{{url('admin/ad/' . $file->id)}}" method="post" enctype="multipart/form-data" >
			<input type="hidden" name="_method" value="put">
			{{csrf_field()}}
			<div class="left ml30 mt30">
				<div class="add_info2 relative">
					<input type="file" name="ad_object_thumb" />
					<div><img src="{{config('web.QINIU_URL')}}/{{$file->ad_object_thumb}}" alt=""></div>
				</div>
			...
			<div class="left ml30 mt20">
				<div class="form_row">
					...
					<input type="text" class="w-300" name="ad_object_name" value="{{$file->ad_object_name}}" />
				</div>
				<div class="form_row mt15">
					<label class="only">显示位置</label>
					@foreach($adplace as $k=>$v)
						<input type="radio" name="ad_place_id" @if($file->ad_place_id == $v->id) checked @else @endif id="" value="{{$v->id}}">{{$v->ad_place_name}}
				...
				<div class="form_row mt15">
					...
					@foreach($adskip as $k=>$v)
						<input type="radio" name="ad_skip_id" id="" value="{{$v->id}}" @if($file->ad_skip_id == $v->id) checked @else @endif title="描述：{{$v->ad_skip_describe}}" dd="{{$v->ad_skip_describe}}">{{$v->ad_skip_name}}
						{{--<a class="btn choose">打开网页</a><a class="btn ml5">用户主页</a><a class="btn ml5">文章详情</a>--}}
					@endforeach
				</div>
				<div class="form_row mt15">
					<label class="only">目标</label>
					<input type="text" class="w-500" name="ad_object_aim" placeholder="" value="{{$file->ad_object_aim}}" />
				</div>
				<div class="form_row mt15">
					<label class="only">显示时间</label>
					<input type="text" class="w-100" id="start" placeholder="开始日期" value="{{date('Y-m-d', $file->ad_start_at)}}" name="ad_start_at" /><label>-</label><input type="text" id="end" name="ad_end_at" class="w-100" value="{{date('Y-m-d', $file->ad_end_at)}}" placeholder="结束日期" />
				</div>
				<div class="form_row">
					<label class="only">排序</label>
					<input type="text" class="w-100" name="ad_object_sort" value="{{$file->ad_object_sort}}" />
				</div>
				<div class="form_row mt15">
