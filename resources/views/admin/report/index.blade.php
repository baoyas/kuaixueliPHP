...
@section('content')
	<div class="main">
		<div class="title_big bline pb10">投诉列表 <span><a href="{{url('/admin/reportGroup')}}">群组投诉</a></span></div>
		...
		<table class="mt15">
			...
			@foreach($data as $v)
				<tr>
				  ...
				  <td><a href="{{url('admin/user/'.$v->report_to_uid)}}">{{$v->to_nickname}}</a></td>
				  <td>
	...
	<script>
		function UpDown (id) {
			layer.confirm('处理将会给被处理者发送系统通知，确认要处理吗？',function(index){
				layer.close(index);
				layer.prompt({title: '被投诉人处理结果！', formType: 2}, function(text, index){
					layer.close(index);
					layer.prompt({title: '投诉人处理结果！', formType: 2}, function(texts, index){
						layer.close(index);
						//此处请求后台程序，下方是成功后的前台处理……
						var url = "{{url('admin/report/updown')}}";
